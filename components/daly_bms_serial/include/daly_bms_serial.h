#ifndef DALY_BMS_SERIAL_H
#define DALY_BMS_SERIAL_H
...
public:
     ...
     *
     * @param uart_port UART port number the BMS is connected to
     */
    Daly_BMS_UART();

    /**
     * @brief Initializes this driver
     * @details Configures the UART peripheral and pre-loads the transmit buffer with command-independent bytes
     */
    bool Init();
     ...
     * @details Populates tempMax, tempMin, and tempAverage in the "get" struct
     * @return True on successful acquisition, false otherwise
     */
    bool getPackTemp();
     ...
     * @brief   Each temperature accounts for 1 byte, according to the
                actual number of temperature sensors, the maximum 21
                bytes, send in 3 frames
    ...
    /**
     * @brief Reset the BMS
     * @details Reset the BMS and let it restart
     */
    bool setBmsReset();
...
private:
     ...
     * @details calculates the checksum and sends the command over the specified UART connection
     */
    ...
    /**
     * @brief Prints out the contents of the RX buffer
     * @details Useful for debugging
     */
    void barfRXBuffer();

    /**
     * @brief UART port number used for communication
     */
